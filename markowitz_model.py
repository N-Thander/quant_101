...
# generating random (different portfolios)
NUM_PORTFOLIOS = 10000

# stocks
stocks = ["AAPL", "WMT", "TSLA", "GE", "AMZN", "DB"]

# Historical date - define start and end date
start_date = "2012-01-01"
end_date = "2026-01-01"


def download_data():
    # name of the stock (key) - stock values (2010 - 2026) (values)
    stock_data = {}

    # Fixed: Removed the redundant nested loop
    for stock in stocks:
...
def show_mean_variance(returns, weights):
    ...
    portfolio_voltility = np.sqrt(
        np.dot(weights.T, np.dot(returns.cov() * NUM_TRADING_DAYS, weights))
    )

    print("Expected portfolio mean (return): ", portfolio_return)
    print("Expected portfolio voltility (standard deviation): ", portfolio_voltility)


def show_portfolios(returns, volatilities):
    plt.figure(figsize=(10, 6))
    plt.scatter(returns, volatilities, c=returns / volatilities, marker="o")
    plt.grid(True)
...
def optimize_portfolio(weights, returns):
    ...
    return optimization.minimize(
        ...
        x0=weights[0],
        args=returns,
        method="SLSQP",  # Fixed: Changed SLSOP to SLSQP
        bounds=bounds,
